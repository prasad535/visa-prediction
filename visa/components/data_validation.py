...
class DataValidation:
    ...
    def validation_number_columns(self, dataframe: DataFrame) -> bool:
        """
        ...
        """
        try:
            logging.info("Entered validate_numbber_of_columns method from data validation component")
    ...
    def is_column_exist(self, df: DataFrame)-> bool:
        ...
        try:
            ...
            if len(missing_categorical_columns) > 0:
                loggin.info(f"Missing categorical columns: {missing_categorical_columns}")
    ...
    def initiate_data_validation(self):
        ...
        try:
            ...
            status = self.validation_number_columns(dataframe=train_df)
            logging.info(f"{train_df.columns}")
            logging.info(f"All required columns present in trainign dataframe: {status}")

            if not status:
                validation_error_msg += f"Columns are missing in training dataframe"
            
            status = self.validation_number_columns(dataframe=test_df)
